...
export function SeriesCard({
  ...
  const genreLine = series.genres.slice(0, 3).join(" · ");
  ...
  return (
    ...
    >
      <div
        className="pointer-events-none absolute inset-x-0 top-0 z-20 h-[3px] bg-gradient-to-r from-emerald-700 via-[var(--cv-accent)] to-transparent opacity-90"
        aria-hidden
      />
      <div
        ref={liftRef}
      ...
      >
        <Link href={`/series/${series.slug}`} className="block outline-none">
          <div className="relative aspect-[185/284] overflow-hidden bg-[var(--cv-deep)]">
            ...
            <div className="pointer-events-none absolute inset-x-0 top-0 z-[8] flex justify-between gap-2 p-2.5">
              <span className="max-w-[min(100%,7.5rem)] truncate rounded-md bg-black/55 px-2 py-0.5 font-[family-name:var(--font-dosis)] text-[10px] font-bold uppercase tracking-wider text-white/95 backdrop-blur-sm ring-1 ring-white/10">
                {series.yearLabel}
              </span>
              <span className="flex shrink-0 items-center gap-0.5 rounded-md bg-black/55 px-2 py-0.5 text-[11px] font-bold tabular-nums text-[var(--cv-star)] backdrop-blur-sm ring-1 ring-white/10 transition-transform duration-500 group-hover:scale-105">
                ★<span className="text-white">{series.rating}</span>
              </span>
            </div>

            <PosterCardDogEar label={readMoreLabel} variant="series" />
            ...
            {labels ? (
              <div className="pointer-events-none absolute inset-0 z-[9] translate-y-3 bg-gradient-to-t from-black/[0.94] via-black/50 to-transparent opacity-0 transition duration-500 ease-out group-hover:translate-y-0 group-hover:opacity-100">
                <div className="absolute inset-x-0 bottom-0 space-y-1.5 p-3 text-left text-[11px] leading-snug text-white/95 transition delay-75 duration-500 group-hover:delay-100">
                  ...
                  {series.starsLine && labels.stars ? (
                    <p className="translate-y-1 opacity-0 transition delay-100 duration-500 group-hover:translate-y-0 group-hover:opacity-100">
                      <span className="text-white/50">{labels.stars}: </span>
                      <span className="line-clamp-2">{series.starsLine}</span>
                    </p>
                  ) : null}
                  {castPreview && labels.cast ? (
                    <p className="translate-y-1 opacity-0 transition delay-100 duration-500 group-hover:translate-y-0 group-hover:opacity-100">
                      <span className="text-white/50">{labels.cast}: </span>
                      <span className="line-clamp-2">{castPreview}</span>
                    </p>
                  ) : null}
                  {series.runtime ? (
          ...
          <div className="relative border-t border-[var(--cv-border)] bg-gradient-to-b from-[var(--cv-card)] to-[color-mix(in_srgb,var(--cv-card)_88%,var(--cv-deep))] px-3 py-3 transition-colors duration-500 group-hover:border-[color-mix(in_srgb,var(--cv-accent)_35%,var(--cv-border))]">
            ...
            <p className="mt-1.5 flex flex-wrap items-center gap-x-1 gap-y-0.5 text-xs text-[var(--cv-muted)] transition duration-500 group-hover:text-[var(--cv-body)]">
              <span
