import type { SeriesTmdbExtras } from "@/lib/catalog";
...
export function SeriesTmdbDetailSection({
  extras,
  labels,
}: {
  extras: SeriesTmdbExtras;
  labels: Labels;
}) {
  return (
    <div className="space-y-10">
      {extras.backdropUrl ? (
        <div className="relative -mx-4 h-48 overflow-hidden rounded-xl border border-[var(--cv-border)] md:-mx-6 md:h-64">
          <Image
            ...
            sizes="100vw"
          />
          <div className="absolute inset-0 bg-gradient-to-t from-[var(--cv-deep)] via-[var(--cv-deep)]/45 to-transparent" />
          {extras.tagline ? (
      ...
      <dl className="grid gap-0 sm:grid-cols-2">
        ...
        <MetaRow
          ...
          value={
            extras.numberOfSeasons > 0 ? String(extras.numberOfSeasons) : null
          }
        />
        <MetaRow
          label={labels.episodes}
          value={
            extras.numberOfEpisodes > 0 ? String(extras.numberOfEpisodes) : null
          }
        />
        <MetaRow
          label={labels.episodeRuntime}
          value={extras.episodeRuntimeLabel}
        />
        <MetaRow label={labels.lastAired} value={extras.lastAirDate || null} />
        <MetaRow label={labels.status} value={extras.status} />
        <MetaRow
          label={labels.networks}
          value={extras.networks.join(", ") || null}
        />
      </dl>
      ...
      {extras.creators.length > 0 ? (
        <section>
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            {extras.creators.map((p) => (
              <Link
                key={p.slug}
                href={`/celebrities/${p.slug}`}
                className="flex items-center gap-2 rounded-lg border border-[var(--cv-border)] bg-[var(--cv-card)] p-2 pr-4 transition hover:border-[var(--cv-accent)]/45"
              >
                <div className="relative h-12 w-12 shrink-0 overflow-hidden rounded-full bg-[var(--cv-deep)]">
                  {p.profileUrl ? (
                    <Image
                      src={p.profileUrl}
                      alt=""
                ...
                <span className="text-sm font-semibold text-[var(--cv-accent)]">
                  {p.name}
                </span>
              </Link>
            ))}
